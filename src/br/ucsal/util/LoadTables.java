...
public class LoadTables {
    ...
    public void creatScherma(Connection c) throws SQLException {
        ...
        // Avalição
        stmt.execute("create table avaliacao(id int PRIMARY KEY, nota int, comentario VARCHAR (150),id_usuario int, id_bibliografia int );");

        // Autores
        stmt.execute("create table autores(nome varchar(20), sobrenome varchar(50),id int PRIMARY KEY);");
        ...
        // Materia
        stmt.execute("create table materias(nome varchar(30),id int PRIMARY KEY);");
        ...
        stmt.execute(
                "alter table usuarios add constraint fk_id_avaliacao foreign key (id_avaliacao) references avaliacao(id);");
        stmt.execute(
